...
5. 複数切り出す場合は「クリップタイトル」を入力して「追加」を押し、別の開始・終了位置でも繰り返します。タイトルが空欄なら`クリップ_1`のような名前が付きます。
6. 最初の追加時に書き出しリストが開きます。登録内容の編集・削除ができ、編集を押すとタイトルと開始・終了位置がメイン画面へ戻ります。
7. 登録済みと同じタイトルを入力するとボタンが「上書き」に変わります。確認後、その項目の開始・終了位置だけを現在の選択範囲で更新します。同名の項目が追加されることはありません。
8. 各項目は`タイトル.mp4`として出力されます。書き出し方法を選び、「書き出し」を押すと、登録順にすべてを書き出します。
