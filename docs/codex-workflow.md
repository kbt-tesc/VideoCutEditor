...
Use `VideoCutEditor.slnx` as the repo's standard solution file for .NET CLI workflows. Keep `VideoCutEditor.sln` in sync as a VS Code compatibility solution because some C# Dev Kit / Roslyn LSP versions can still miss `ProjectReference` resolution from `.slnx` and report false DocumentCompilerSemantic errors for namespaces such as `VideoCutEditor.Core`. Keep `.vscode/settings.json` pinned to `VideoCutEditor.sln` with `dotnet.defaultSolution` for editor diagnostics. If VS Code still reports stale warnings after these files change, reload the VS Code window or restart the C# language server before assuming the code is broken.
