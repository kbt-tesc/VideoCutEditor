...
- Show encoder information, media information, and export logs in a non-modal INFO window with its own scrolling area so logs do not expand or block the main editor layout. Reuse the open INFO window instead of opening duplicates, and keep its displayed values synchronized while export continues.
