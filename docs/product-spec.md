...
- Show encoder information, media information, and export logs in a non-modal INFO window owned by the main window, with its own scrolling area so logs do not expand or block the main editor layout. Reuse the open INFO window instead of opening duplicates, keep it associated with the main window for normal stacking/minimize behavior, and keep its displayed values synchronized while export continues.
