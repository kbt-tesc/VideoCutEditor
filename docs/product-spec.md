...
- Show an `音声を再エンコード` checkbox only when the source has audio. Audio encoding becomes mandatory and the checked control is disabled when normalization, an audio fade, or destination-container incompatibility requires it; otherwise the user may explicitly enable it. Show a compact audio-bitrate input only while audio encoding is active, and place the horizontal VBR/CBR choices on the following row only for WebM Opus output so the controls remain complete at narrow panel widths.
