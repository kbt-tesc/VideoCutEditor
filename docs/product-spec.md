...
- A blank title is assigned a one-based placeholder such as `クリップ_1`. Registered titles and existing destination MP4 or WebM files are not reused; a numeric suffix is added when needed.
- A nonblank title that matches a registered clip is not added as another row. The registration button changes from `追加` to `上書き`; activating it asks for confirmation, and approval replaces only that clip's start/end range.
- Each export-list row provides an edit action. Editing restores the row's title and start/end range to the main timeline controls and activates the main window.
- The main export surface provides an output-folder open button beside the planned output controls.
- Show encoder information, media information, export progress, and export logs in a non-modal INFO window owned by the main window, with its own scrolling area so logs do not expand or block the main editor layout. Open or reactivate the INFO window automatically when export starts. Media information identifies HDR10/PQ or HLG and shows the probed color space, transfer characteristic, and color primaries. Reuse the open INFO window instead of opening duplicates, keep it associated with the main window for normal stacking/minimize behavior, and keep its displayed values synchronized while export continues.
- Lay out the INFO window as three full-width rows in this order: one combined encoder/media information box, the export log, and the export progress area fixed at the bottom while active. Only the export-log row grows or shrinks with window height; all rows stretch with window width. Put an `自動スクロール` checkbox at the right of the export-log heading, default it on without persisting the choice, and keep the log at its newest line while enabled.
...
- Re-encode mode offers MP4 and WebM output. WebM uses AV1 video; compatible Opus/Vorbis audio may be copied when no audio processing is requested, while audio that requires encoding uses Opus. WebM is available only when the required ffmpeg encoders are detected.
- Fast copy shows the MP4/WebM container selector only for MP4 or WebM sources whose probed video and non-audio stream set can be copied into the other container. Incompatible audio may be converted to AAC for MP4 or Opus for WebM when the required encoder is detected. If compatibility cannot be established, keep the source container selected and hide the switch.
...
- During export, show the active progress bar in the INFO window rather than the main export settings pane. Video processing shows processed frames and estimated total frames. Audio-only processing, including loudness analysis, switches the same INFO progress area to an audio progress bar based on processed time. Keep current status and cancellation on the main window and detailed logs in INFO.
...
- Normalize audio is a setting available in both Fast copy and Re-encode. It uses two-pass loudness normalization to `-14 LUFS`, re-encodes audio to AAC for MP4 or Opus for WebM, preserves video according to the selected mode where practical, and reports a clear error when probed media has no audio stream.
- Show an `音声を再エンコード` checkbox only when the source has audio. Audio encoding becomes mandatory and the checked control is disabled when normalization, an audio fade, or destination-container incompatibility requires it; otherwise the user may explicitly enable it. Show a compact audio-bitrate input only while audio encoding is active. For WebM Opus, keep the bitrate input and horizontal VBR/CBR choices on one row by using compact radio-button widths that fit the export panel.
- Audio encoding uses bitrate-targeted AAC for MP4 and Opus for WebM. Default the bitrate field to the probed source bitrate for AAC or Opus inputs, including after output-container changes; use `128 kbps` for other or unknown source audio codecs, with VBR as the WebM Opus default.
...
- Registered clips are exported as `<title>.mp4` or `<title>.webm`, according to the selected output container, in registration order.
...
- MP4 and WebM inputs default to their source container. Re-encode allows selecting MP4 or WebM. Fast copy allows switching when ffprobe metadata proves mapped video and retained non-audio streams are compatible and any incompatible audio can be re-encoded with an available destination codec.
