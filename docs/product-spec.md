...
- Re-encode mode offers MP4 and WebM output. WebM uses AV1 video and Opus audio, and is available only when the required ffmpeg encoders are detected.
- Fast copy shows the MP4/WebM container selector only for MP4 or WebM sources whose complete probed stream set can be copied into the other container. If compatibility cannot be established, keep the source container selected and hide the switch.
...
- MP4 and WebM inputs default to their source container. Re-encode allows selecting MP4 or WebM. Fast copy allows switching between those containers only when ffprobe metadata proves every mapped stream is compatible with the destination.
